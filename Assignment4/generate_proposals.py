...
annotation_folder = './training/Annotations'
data_folder = './training/JPEGImages'
...
for f in files:
    ...
    cv2.setNumThreads(4);
    img = cv2.imread(data_folder + "/" + img_name)
    ...
    for i, rect in enumerate(rects):
        ...
        for j in range(len(labels)):
            if IoU(np.array(bbox[j]), np.array([x, y, x + w, y + h])) >= 0.5:
                cropped_image.save(train_data + "/" + labels[j] + "/" + str(cnt) + ".jpg")
                cnt = cnt + 1
